...
(async () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (proofreaderAPISupported) {
            ...
            ({ corrected, corrections } = await proofreader.proofread(
                input.innerText
            ));
        } else {
            // Use fake data.
            const response = await fetch('/proofread', { method: 'POST', headers: { 'Content-type': 'application/json' }, body: JSON.stringify({ "input": input.innerText }) });
            ({ corrected, corrections } = await response.json());
        }
